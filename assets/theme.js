...
function focusHash() {
  ...
  if (hash && document.getElementById(hash.slice(1))) {
    this.pageLinkFocus((0, _jquery.default)(hash));
  }
}
/**
...
function bindInPageLinks() {
  ...
  (0, _jquery.default)('a[href*=#]').on('click', function (evt) {
    _this.pageLinkFocus((0, _jquery.default)(evt.currentTarget.hash));
  });
}
/**
...
function trapFocus(options) {
  ...
  (0, _jquery.default)(document).on(eventName, function (evt) {
...
var _base = _interopRequireDefault(require("../sections/base"));
...
var SectionManager = /*#__PURE__*/function () {
  ...
  _createClass(SectionManager, [{
    ...
    value: function getSingleInstance(type) {
      var instance;
    ...
    key: "load",
    value: function load(container, constructor) {
      var $container = (0, _jquery.default)(container);
      var id = $container.attr(SECTION_ID_ATTR);
      var type = $container.attr(SECTION_TYPE_ATTR);
      var Konstructor = constructor || this.constructors[type]; // No param re-assignment

      if (typeof Konstructor === 'undefined') {
        return;
      }

      var instance = _jquery.default.extend(new Konstructor(container), {
        id: id,
        type: type,
        container: container
      });
    ...
    key: "onSectionLoad",
    value: function onSectionLoad(e) {
      var container = (0, _jquery.default)("[".concat(SECTION_ID_ATTR, "]"), e.target)[0];
    ...
    key: "onSectionUnload",
    value: function onSectionUnload(e) {
      var instance = this.getInstanceById(e.detail.sectionId);
    ...
    key: "onGenericEvent",
    value: function onGenericEvent(e, func) {
      var instance = this.getInstanceById(e.detail.sectionId);
    ...
    value: function register(type, constructor) {
      ...
      if (!(constructor.prototype instanceof _base.default)) {
        return;
      }

      this.constructors[type] = constructor;
      (0, _jquery.default)("[".concat(SECTION_TYPE_ATTR, "=").concat(type, "]")).each(function (index, container) {
        _this.load(container, constructor);
      });
    }
  }]);
...
exports.getUrlWithUpdatedQueryStringParameter = getUrlWithUpdatedQueryStringParameter;
exports.getUrlWithRemovedQueryStringParameter = getUrlWithRemovedQueryStringParameter;
exports.isThemeEditor = isThemeEditor;
exports.whichTransitionEnd = whichTransitionEnd;
exports.userAgentBodyClass = userAgentBodyClass;
...
function getQueryString() {
  var queryString = window.location.search && window.location.search.substr(1) || ''; // Add the '?' prefix if there is an actual query
...
function getUrlWithUpdatedQueryStringParameter(key, value, _uri) {
  var uri = _uri || window.location.href; // remove the hash part before operating on the uri

  var i = uri.indexOf('#');
  var hash = i === -1 ? '' : uri.substr(i);
  uri = i === -1 ? uri : uri.substr(0, i); // eslint-disable-line no-param-reassign

  var re = new RegExp('([?&])' + key + '=.*?(&|$)', 'i'); // eslint-disable-line prefer-template
...
function getUrlWithRemovedQueryStringParameter(parameterKeyToRemove, _uri) {
  var uri = _uri || window.location.href;
  var rtn = uri.split('?')[0];
  var param;
  var paramsArr = [];
  var queryString = uri.indexOf('?') !== -1 ? uri.split('?')[1] : '';
...
function userAgentBodyClass() {
  ...
  var classes = d.className;
  var matches; // Detect iOS (needed to disable zoom on form elements)
...
function pluralize(num, singular, plural) {
  ...
  if (number === 1) {
    ...
    if (typeof plural === 'undefined') {
      output = "".concat(singular, "s"); // last resort, turn singular into a plural
    }
  }
...
var BaseSection = /*#__PURE__*/function () {
  function BaseSection(container, name) {
    _classCallCheck(this, BaseSection);

    this.$container = container instanceof _jquery.default ? container : (0, _jquery.default)(container);
    this.id = this.$container.data('section-id');
    this.type = this.$container.data('section-type');
    this.name = name;
    this.namespace = ".".concat(this.name);
    this.events = {
      SCROLL: "scroll".concat(this.namespace),
      CLICK: "click".concat(this.namespace),
      RESIZE: "resize".concat(this.namespace),
      MOUSEENTER: "mouseenter".concat(this.namespace),
...
var CollectionSection = /*#__PURE__*/function (_BaseSection) {
  _inherits(CollectionSection, _BaseSection);

  var _super = _createSuper(CollectionSection);
  ...
  function CollectionSection(container) {
    ...
    _this = _super.call(this, container, 'collection');

    _this.$container.find('[data-product-card]').map(function (i, el) {
      new _productCard.default(el);
    });
...
var _productDetailForm = _interopRequireDefault(require("../components/product/productDetailForm"));

var _productDetailGallery = _interopRequireDefault(require("../components/product/productDetailGallery"));
...
var ProductSection = /*#__PURE__*/function (_BaseSection) {
  _inherits(ProductSection, _BaseSection);

  var _super = _createSuper(ProductSection);
  ...
  function ProductSection(container) {
    ...
    _this = _super.call(this, container, 'product');
    _this.productDetailForm = new _productDetailForm.default((0, _jquery.default)('[data-product-detail-form]', _this.$container).first());
...
(function () {
  ...
  (0, _a11y.pageLinkFocus)((0, _jquery.default)(window.location.hash));
  (0, _utils.userAgentBodyClass)();
  window.addEventListener('resize', (0, _throttleDebounce.throttle)(100, setViewportHeightProperty));
